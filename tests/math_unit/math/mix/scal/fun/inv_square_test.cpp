...
TEST(AgradFwdInvSquare, FvarVar_1stDeriv) {
  ...
  EXPECT_DOUBLE_EQ(inv_square(0.5), a.val_.val());
  EXPECT_DOUBLE_EQ(-2.0 * inv_square(0.5) / (0.5), a.d_.val());

  AVEC y = createAVEC(x.val_);
  VEC g;
  a.val_.grad(y, g);
  EXPECT_DOUBLE_EQ(-2.0 / (0.5 * 0.5 * 0.5), g[0]);
}
TEST(AgradFwdInvSquare, FvarVar_2ndDeriv) {
  ...
  VEC g;
  a.d_.grad(y, g);
  EXPECT_DOUBLE_EQ(-2.0 * -3.0 / (0.5 * 0.5 * 0.5 * 0.5), g[0]);
}
...
TEST(AgradFwdInvSquare, FvarFvarVar_1stDeriv) {
  ...
  EXPECT_DOUBLE_EQ(inv_square(0.5), a.val_.val_.val());
  EXPECT_DOUBLE_EQ(-2.0 * inv_square(0.5) / (0.5), a.val_.d_.val());
  ...
  EXPECT_DOUBLE_EQ(-2.0 * inv_square(0.5) / (0.5), g[0]);
}
