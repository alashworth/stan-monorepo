...
TEST_F(AgradFwdLogit, FvarVar_1stDeriv) {
  ...
  EXPECT_DOUBLE_EQ(logit(0.5), a.val_.val());
  EXPECT_DOUBLE_EQ(1.3 / (0.5 - 0.25), a.d_.val());

  AVEC y = createAVEC(x.val_);
  VEC g;
  a.val_.grad(y, g);
  EXPECT_DOUBLE_EQ(1 / (0.5 - 0.25), g[0]);
}
...
TEST_F(AgradFwdLogit, FvarFvarVar_1stDeriv) {
  ...
  EXPECT_DOUBLE_EQ(logit(0.5), a.val_.val_.val());
  EXPECT_DOUBLE_EQ(1 / (0.5 - 0.25), a.val_.d_.val());
  ...
  EXPECT_DOUBLE_EQ(1 / (0.5 - 0.25), a.d_.val_.val());
  EXPECT_DOUBLE_EQ(0, a.d_.d_.val());

  AVEC q = createAVEC(y.val_.val_);
  VEC r;
  b.val_.val_.grad(q, r);
  EXPECT_DOUBLE_EQ(1 / (0.5 - 0.25), r[0]);
}
...
TEST_F(AgradFwdLogit, logit_NaN_1) {
  logit_fun logit_;
