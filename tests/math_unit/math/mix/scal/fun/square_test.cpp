...
TEST(AgradFwdSquare, FvarVar_1stDeriv) {
  ...
  EXPECT_DOUBLE_EQ(square(1.5), a.val_.val());
  EXPECT_DOUBLE_EQ(1.3 * 2.0 * (1.5), a.d_.val());

  AVEC y = createAVEC(x.val_);
  VEC g;
  a.val_.grad(y, g);
  EXPECT_DOUBLE_EQ(2.0 * (1.5), g[0]);
}
...
TEST(AgradFwdSquare, FvarFvarVar_1stDeriv) {
  ...
  EXPECT_DOUBLE_EQ(square(1.5), a.val_.val_.val());
  EXPECT_DOUBLE_EQ(2.0 * 2.0 * (1.5), a.val_.d_.val());
  ...
  EXPECT_DOUBLE_EQ(square(1.5), b.val_.val_.val());
  EXPECT_DOUBLE_EQ(0, b.val_.d_.val());
  EXPECT_DOUBLE_EQ(2.0 * 2.0 * (1.5), b.d_.val_.val());
  EXPECT_DOUBLE_EQ(0, b.d_.d_.val());

  AVEC q = createAVEC(y.val_.val_);
  VEC r;
  b.val_.val_.grad(q, r);
  EXPECT_DOUBLE_EQ(2.0 * 1.5, r[0]);
}
...
TEST(AgradFwdSquare, square_NaN_1) {
  square_fun square_;
