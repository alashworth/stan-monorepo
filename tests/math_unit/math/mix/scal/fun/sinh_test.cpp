...
TEST_F(AgradFwdSinh, FvarVar_1stDeriv) {
  ...
  EXPECT_DOUBLE_EQ(sinh(1.5), a.val_.val());
  EXPECT_DOUBLE_EQ(1.3 * cosh(1.5), a.d_.val());

  AVEC y = createAVEC(x.val_);
  VEC g;
  a.val_.grad(y, g);
  EXPECT_DOUBLE_EQ(cosh(1.5), g[0]);
}
...
TEST_F(AgradFwdSinh, FvarVar_2ndDeriv) {
  ...
  VEC g;
  a.d_.grad(y, g);
  EXPECT_DOUBLE_EQ(1.3 * sinh(1.5), g[0]);
}
...
TEST_F(AgradFwdSinh, FvarFvarVar_1stDeriv) {
  ...
  EXPECT_DOUBLE_EQ(sinh(1.5), a.val_.val_.val());
  EXPECT_DOUBLE_EQ(2.0 * cosh(1.5), a.val_.d_.val());
  ...
  EXPECT_DOUBLE_EQ(2.0 * cosh(1.5), a.d_.val_.val());
  EXPECT_DOUBLE_EQ(0, a.d_.d_.val());

  AVEC q = createAVEC(y.val_.val_);
  VEC r;
  b.val_.val_.grad(q, r);
  EXPECT_DOUBLE_EQ(cosh(1.5), r[0]);
}
...
TEST_F(AgradFwdSinh, FvarFvarVar_2ndDeriv) {
  ...
  a.val_.d_.grad(p, g);
  stan::math::recover_memory();
  EXPECT_DOUBLE_EQ(2.0 * sinh(1.5), g[0]);
  ...
  VEC r;
  b.d_.val_.grad(q, r);
  EXPECT_DOUBLE_EQ(2.0 * sinh(1.5), r[0]);
}
...
TEST_F(AgradFwdSinh, FvarFvarVar_3rdDeriv) {
  ...
  VEC g;
  a.d_.d_.grad(p, g);
  EXPECT_DOUBLE_EQ(2.352409615243247325767667965442, g[0]);
}
