...
TEST(AgradFwdLgamma, FvarVar_1stDeriv) {
  ...
  EXPECT_DOUBLE_EQ(lgamma(0.5), a.val_.val());
  EXPECT_DOUBLE_EQ(1.3 * digamma(0.5), a.d_.val());

  AVEC y = createAVEC(x.val_);
  VEC g;
  a.val_.grad(y, g);
  EXPECT_DOUBLE_EQ(digamma(0.5), g[0]);
}
TEST(AgradFwdLgamma, FvarVar_2ndDeriv) {
  ...
  VEC g;
  a.d_.grad(y, g);
  EXPECT_DOUBLE_EQ(1.3 * 4.9348022, g[0]);
}
TEST(AgradFwdLgamma, FvarFvarVar_1stDeriv) {
  ...
  EXPECT_DOUBLE_EQ(lgamma(0.5), a.val_.val_.val());
  EXPECT_DOUBLE_EQ(digamma(0.5), a.val_.d_.val());
  ...
  EXPECT_DOUBLE_EQ(lgamma(0.5), b.val_.val_.val());
  EXPECT_DOUBLE_EQ(0, b.val_.d_.val());
  EXPECT_DOUBLE_EQ(digamma(0.5), b.d_.val_.val());
  EXPECT_DOUBLE_EQ(0, b.d_.d_.val());

  AVEC q = createAVEC(y.val_.val_);
  VEC r;
  b.val_.val_.grad(q, r);
  EXPECT_DOUBLE_EQ(digamma(0.5), r[0]);
}
TEST(AgradFwdLgamma, FvarFvarVar_2ndDeriv) {
  ...
  VEC g;
  a.val_.d_.grad(p, g);
  EXPECT_DOUBLE_EQ(4.9348022, g[0]);
  ...
  VEC r;
  b.d_.val_.grad(q, r);
  EXPECT_DOUBLE_EQ(4.9348022, r[0]);
}
TEST(AgradFwdLgamma, FvarFvarVar_3rdDeriv) {
  ...
  VEC g;
  a.d_.d_.grad(p, g);
  EXPECT_DOUBLE_EQ(-16.8287966442343199955963342612, g[0]);
}
