...
TEST(AgradMixMatrixFill, fv) {
  ...
  fvar<var> y = 10;
  fill(x, y);
  EXPECT_DOUBLE_EQ(10.0, x.val_.val());

  std::vector<fvar<var> > z(2);
  fvar<var> a = 15;
  fill(z, a);
  EXPECT_DOUBLE_EQ(15.0, z[0].val_.val());
  EXPECT_DOUBLE_EQ(15.0, z[1].val_.val());
  EXPECT_EQ(2U, z.size());
  ...
  for (int i = 0; i < 2; ++i)
    for (int j = 0; j < 3; ++j)
      EXPECT_DOUBLE_EQ(12.0, m(i, j).val_.val());

  Matrix<fvar<var>, Dynamic, 1> rv(3);
  fill(rv, fvar<var>(13));
  for (int i = 0; i < 3; ++i)
    EXPECT_DOUBLE_EQ(13.0, rv(i).val_.val());

  Matrix<fvar<var>, 1, Dynamic> v(4);
  fill(v, fvar<var>(22));
  for (int i = 0; i < 4; ++i)
    EXPECT_DOUBLE_EQ(22.0, v(i).val_.val());

  vector<vector<fvar<var> > > d(3, vector<fvar<var> >(2));
  fill(d, fvar<var>(54));
  for (size_t i = 0; i < 3; ++i)
    for (size_t j = 0; j < 2; ++j)
      EXPECT_DOUBLE_EQ(54, d[i][j].val_.val());
}
TEST(AgradMixMatrixFill, fv2) {
  ...
  fill(y, 3.0);
  EXPECT_EQ(3, y.size());
  EXPECT_DOUBLE_EQ(3.0, y[0].val_.val());
}
...
TEST(AgradMixMatrixFill, ffv) {
  ...
  fvar<fvar<var> > y = 10;
  fill(x, y);
  EXPECT_DOUBLE_EQ(10.0, x.val_.val_.val());

  std::vector<fvar<fvar<var> > > z(2);
  fvar<fvar<var> > a = 15;
  fill(z, a);
  EXPECT_DOUBLE_EQ(15.0, z[0].val_.val_.val());
  EXPECT_DOUBLE_EQ(15.0, z[1].val_.val_.val());
  EXPECT_EQ(2U, z.size());
...
TEST(AgradMixMatrixFill, ffv2) {
  ...
  fill(y, 3.0);
  EXPECT_EQ(3, y.size());
  EXPECT_DOUBLE_EQ(3.0, y[0].val_.val_.val());
}
