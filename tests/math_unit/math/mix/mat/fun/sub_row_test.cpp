...
TEST(AgradMixMatrixSubRow, SubRow6_matrix_fv) {
  ...
  EXPECT_EQ(2, v.size());
  for (int i = 0; i < 2; ++i) {
    EXPECT_DOUBLE_EQ(m(0, 1 + i).val_.val(), v(i).val_.val());
    EXPECT_DOUBLE_EQ(m(0, 1 + i).d_.val(), v(i).d_.val());
  }
}
...
TEST(AgradMixMatrixSubRow, SubRow6_matrix_ffv) {
  ...
  EXPECT_EQ(2, v.size());
  for (int i = 0; i < 2; ++i) {
    EXPECT_DOUBLE_EQ(m(0, 1 + i).val_.val_.val(), v(i).val_.val_.val());
    EXPECT_DOUBLE_EQ(m(0, 1 + i).d_.val_.val(), v(i).d_.val_.val());
  }
}
