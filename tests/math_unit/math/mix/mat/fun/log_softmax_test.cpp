...
TEST(AgradMixMatrixLogSoftmax, fv_1stDeriv) {
  ...
  EXPECT_DOUBLE_EQ(0.99998331, h[0]);
  EXPECT_DOUBLE_EQ(-0.00012339251, h[1]);
  EXPECT_DOUBLE_EQ(-0.99985993, h[2]);
}
...
TEST(AgradMixMatrixLogSoftmax, ffv_1stDeriv) {
  ...
  EXPECT_EQ(1, theta.size());
  EXPECT_DOUBLE_EQ(0.0, theta[0].val_.val().val());
  ...
  EXPECT_EQ(2, theta2.size());
  EXPECT_DOUBLE_EQ(-2.1269281, theta2[0].val_.val().val());
  EXPECT_DOUBLE_EQ(-0.12692802, theta2[1].val_.val().val());
  EXPECT_DOUBLE_EQ(0.88079709, theta2[0].d_.val().val());
  EXPECT_DOUBLE_EQ(-0.11920292, theta2[1].d_.val().val());
  ...
  EXPECT_EQ(3, theta3.size());
  EXPECT_DOUBLE_EQ(-11.00014, theta3[0].val_.val().val());
  EXPECT_DOUBLE_EQ(-9.0001402, theta3[1].val_.val().val());
  EXPECT_DOUBLE_EQ(-0.00014010169, theta3[2].val_.val().val());
  EXPECT_DOUBLE_EQ(0.99998331, theta3[0].d_.val().val());
  EXPECT_DOUBLE_EQ(-1.6699361e-05, theta3[1].d_.val().val());
  EXPECT_DOUBLE_EQ(-1.6699361e-05, theta3[2].d_.val().val());
