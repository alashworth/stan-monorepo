...
TEST(AgradFwdModifiedBesselFirstKind, Fvar) {
  ...
  EXPECT_DOUBLE_EQ(9.7594651537044499094, x.val_);
  EXPECT_DOUBLE_EQ(8.8620556637102180189, x.d_);

  fvar<double> c(-3.0, 2.0);

  x = modified_bessel_first_kind(1, c);
  EXPECT_DOUBLE_EQ(-3.9533702174026093964, x.val_);
  EXPECT_DOUBLE_EQ(2.0 * 3.563002513397487620, x.d_);

  x = modified_bessel_first_kind(-1, c);
  EXPECT_DOUBLE_EQ(-3.95337021740260939647, x.val_);
  EXPECT_DOUBLE_EQ(2.0 * 3.5630025133974876, x.d_);
}
...
TEST(AgradFwdModifiedBesselFirstKind, FvarFvarDouble) {
  ...
  EXPECT_DOUBLE_EQ(modified_bessel_first_kind(1, 4.0), a.val_.val_);
  EXPECT_DOUBLE_EQ(0, a.val_.d_);
  EXPECT_DOUBLE_EQ(8.8620556637102180189, a.d_.val_);
  EXPECT_DOUBLE_EQ(0, a.d_.d_);

  fvar<fvar<double> > b = modified_bessel_first_kind(1, x);

  EXPECT_DOUBLE_EQ(modified_bessel_first_kind(1, 4.0), b.val_.val_);
  EXPECT_DOUBLE_EQ(8.8620556637102180189, b.val_.d_);
...
TEST(AgradFwdModifiedBesselFirstKind, modified_bessel_first_kind_NaN_0) {
