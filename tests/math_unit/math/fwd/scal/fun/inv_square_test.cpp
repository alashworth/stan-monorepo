...
TEST(AgradFwdInvSquare, Fvar) {
  ...
  EXPECT_DOUBLE_EQ(inv_square(0.5), a.val_);
  EXPECT_DOUBLE_EQ(-2 / (0.5 * 0.5 * 0.5), a.d_);
...
TEST(AgradFwdInvSquare, FvarFvarDouble) {
  ...
  EXPECT_DOUBLE_EQ(inv_square(0.5), a.val_.val_);
  EXPECT_DOUBLE_EQ(-2.0 * inv_square(0.5) / (0.5), a.val_.d_);
