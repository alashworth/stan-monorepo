...
TEST_F(AgradFwdAsinh, Fvar) {
  ...
  EXPECT_DOUBLE_EQ(asinh(0.5), a.val_);
  EXPECT_DOUBLE_EQ(1 / sqrt(1 + (0.5) * (0.5)), a.d_);

  fvar<double> y(-1.2, 1.0);

  fvar<double> b = asinh(y);
  EXPECT_DOUBLE_EQ(asinh(-1.2), b.val_);
  EXPECT_DOUBLE_EQ(1 / sqrt(1 + (-1.2) * (-1.2)), b.d_);

  fvar<double> c = asinh(-x);
  EXPECT_DOUBLE_EQ(asinh(-0.5), c.val_);
...
TEST_F(AgradFwdAsinh, FvarFvarDouble) {
  ...
  EXPECT_DOUBLE_EQ(asinh(1.5), a.val_.val_);
  EXPECT_DOUBLE_EQ(2.0 / sqrt(1.0 + 1.5 * 1.5), a.val_.d_);
  ...
  EXPECT_DOUBLE_EQ(2.0 / sqrt(1.0 + 1.5 * 1.5), a.d_.val_);
  EXPECT_DOUBLE_EQ(0, a.d_.d_);
}
struct asinh_fun {
