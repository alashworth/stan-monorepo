...
TEST(AgradFwdInvCLogLog, Fvar) {
  ...
  EXPECT_DOUBLE_EQ(inv_cloglog(0.5), a.val_);
  EXPECT_DOUBLE_EQ(exp(0.5 - exp(0.5)), a.d_);

  fvar<double> y(-1.2, 1.0);

  fvar<double> b = inv_cloglog(y);
  EXPECT_DOUBLE_EQ(inv_cloglog(-1.2), b.val_);
  EXPECT_DOUBLE_EQ(exp(-1.2 - exp(-1.2)), b.d_);

  fvar<double> z(1.5, 2.0);

  fvar<double> c = inv_cloglog(z);
  EXPECT_DOUBLE_EQ(inv_cloglog(1.5), c.val_);
  EXPECT_DOUBLE_EQ(2.0 * exp(1.5 - exp(1.5)), c.d_);
}
...
TEST(AgradFwdInvCLogLog, FvarFvarDouble) {
  ...
  EXPECT_DOUBLE_EQ(inv_cloglog(0.5), a.val_.val_);
  EXPECT_DOUBLE_EQ(exp(0.5 - exp(0.5)), a.val_.d_);
  ...
  EXPECT_DOUBLE_EQ(exp(0.5 - exp(0.5)), a.d_.val_);
  EXPECT_DOUBLE_EQ(0, a.d_.d_);
}
