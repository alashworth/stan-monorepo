...
TEST(AgradFwdFmin, Fvar) {
  ...
  EXPECT_DOUBLE_EQ(-3.0, a.val_);
  EXPECT_DOUBLE_EQ(2.0, a.d_);

  fvar<double> b = fmin(2 * x, y);
  EXPECT_DOUBLE_EQ(-3.0, b.val_);
  EXPECT_DOUBLE_EQ(2.0, b.d_);

  fvar<double> c = fmin(y, x);
  EXPECT_DOUBLE_EQ(-3.0, c.val_);
  ...
  EXPECT_DOUBLE_EQ(1.0, e.val_);
  EXPECT_DOUBLE_EQ(0.0, e.d_);

  fvar<double> f = fmin(z, x);
  EXPECT_DOUBLE_EQ(1.0, f.val_);
...
TEST(AgradFwdFmin, FvarFvarDouble) {
  ...
  EXPECT_DOUBLE_EQ(fmin(2.5, 1.5), a.val_.val_);
