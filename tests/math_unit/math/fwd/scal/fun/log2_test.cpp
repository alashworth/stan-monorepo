...
TEST(AgradFwdLog2, Fvar) {
  ...
  EXPECT_DOUBLE_EQ(log2(0.5), a.val_);
  EXPECT_DOUBLE_EQ(1 / (0.5 * log(2)), a.d_);

  fvar<double> b = 2 * log2(x) + 4;
  EXPECT_DOUBLE_EQ(2 * log2(0.5) + 4, b.val_);
  EXPECT_DOUBLE_EQ(2 / (0.5 * log(2)), b.d_);

  fvar<double> c = -log2(x) + 5;
  EXPECT_DOUBLE_EQ(-log2(0.5) + 5, c.val_);
  EXPECT_DOUBLE_EQ(-1 / (0.5 * log(2)), c.d_);

  fvar<double> d = -3 * log2(x) + 5 * x;
  EXPECT_DOUBLE_EQ(-3 * log2(0.5) + 5 * 0.5, d.val_);
  EXPECT_DOUBLE_EQ(-3 / (0.5 * log(2)) + 5, d.d_);
...
TEST(AgradFwdLog2, FvarFvarDouble) {
  ...
  EXPECT_DOUBLE_EQ(log2(0.5), a.val_.val_);
  EXPECT_DOUBLE_EQ(1 / (0.5 * log(2)), a.val_.d_);
  ...
  EXPECT_DOUBLE_EQ(1 / (0.5 * log(2)), a.d_.val_);
  EXPECT_DOUBLE_EQ(0, a.d_.d_);
}
