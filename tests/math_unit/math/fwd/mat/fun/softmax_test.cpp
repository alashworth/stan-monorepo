...
TEST(AgradFwdMatrixSoftmax, fd) {
  ...
  EXPECT_DOUBLE_EQ(exp(-1) / (exp(-1) + exp(1)), theta2[0].val_);
  EXPECT_DOUBLE_EQ(exp(1) / (exp(-1) + exp(1)), theta2[1].val_);
  EXPECT_DOUBLE_EQ(0.10499358, theta2[0].d_);
  EXPECT_DOUBLE_EQ(-0.10499358, theta2[1].d_);
  ...
  EXPECT_DOUBLE_EQ(exp(-1) / (exp(-1) + exp(1) + exp(10.0)), theta3[0].val_);
  EXPECT_DOUBLE_EQ(exp(1) / (exp(-1) + exp(1) + exp(10.0)), theta3[1].val_);
  EXPECT_DOUBLE_EQ(exp(10) / (exp(-1) + exp(1) + exp(10.0)), theta3[2].val_);
  EXPECT_DOUBLE_EQ(1.6699081e-05, theta3[0].d_);
  EXPECT_DOUBLE_EQ(-2.0605762e-09, theta3[1].d_);
  EXPECT_DOUBLE_EQ(-1.6697022e-05, theta3[2].d_);
}
