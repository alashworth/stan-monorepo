...
TEST(AgradFwdMatrixSubRow, SubRow6_matrix_fd) {
  ...
  EXPECT_EQ(2, v.size());
  for (int i = 0; i < 2; ++i) {
    EXPECT_DOUBLE_EQ(m(0, 1 + i).val_, v(i).val_);
    EXPECT_DOUBLE_EQ(m(0, 1 + i).d_, v(i).d_);
  }
}
...
TEST(AgradFwdMatrixSubRow, SubRow6_matrix_ffd) {
  ...
  EXPECT_EQ(2, v.size());
  for (int i = 0; i < 2; ++i) {
    EXPECT_DOUBLE_EQ(m(0, 1 + i).val_.val_, v(i).val_.val_);
    EXPECT_DOUBLE_EQ(m(0, 1 + i).d_.val_, v(i).d_.val_);
  }
}
