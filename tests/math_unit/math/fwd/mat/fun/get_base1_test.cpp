...
TEST(AgradFwdMatrixGetBase1, failing_pre_20_fd) {
  ...
  y << 1, 2, 3;
  EXPECT_DOUBLE_EQ(1, get_base1(y, 1, "y", 1).val_);
}
TEST(AgradFwdMatrixGetBase1, get_base1_vec1_fd) {
  ...
  x[0] = 10.0;
  x[1] = 20.0;
  EXPECT_DOUBLE_EQ(10.0, get_base1(x, 1, "x[1]", 0).val_);
  EXPECT_DOUBLE_EQ(20.0, get_base1(x, 2, "x[1]", 0).val_);
...
TEST(AgradFwdMatrixGetBase1, get_base1_matrix_fd) {
  ...
  for (size_t i = 0; i < 4; ++i) {
    for (size_t j = 0; j < 3; ++j) {
      EXPECT_DOUBLE_EQ(x(i, j).val_, get_base1(x, i + 1, j + 1, "x", 1).val_);
      EXPECT_DOUBLE_EQ(x(i, j).val_, get_base1(x, i + 1, "x", 1)(0, j).val_);
      Matrix<fvar<double>, 1, Dynamic> xi
          = get_base1<fvar<double> >(x, i + 1, "x", 1);
      EXPECT_DOUBLE_EQ(x(i, j).val_, xi[j].val_);
      EXPECT_DOUBLE_EQ(x(i, j).val_, get_base1(xi, j + 1, "xi", 2).val_);
    }
  }
...
TEST(AgradFwdMatrixGetBase1, get_base1_vector_fd) {
  ...
  for (size_t i = 0; i < 3; ++i)
    EXPECT_DOUBLE_EQ(x(i).val_, get_base1(x, i + 1, "x", 1).val_);
  EXPECT_THROW(get_base1(x, 0, "x", 1), std::out_of_range);
...
TEST(AgradFwdMatrixGetBase1, failing_pre_20_ffd) {
  ...
  y << 1, 2, 3;
  EXPECT_DOUBLE_EQ(1, get_base1(y, 1, "y", 1).val_.val_);
}
TEST(AgradFwdMatrixGetBase1, get_base1_vec1_ffd) {
  ...
  x[0] = 10.0;
  x[1] = 20.0;
  EXPECT_DOUBLE_EQ(10.0, get_base1(x, 1, "x[1]", 0).val_.val_);
  EXPECT_DOUBLE_EQ(20.0, get_base1(x, 2, "x[1]", 0).val_.val_);
...
TEST(AgradFwdMatrixGetBase1, get_base1_vector_ffd) {
  ...
  for (size_t i = 0; i < 3; ++i)
    EXPECT_DOUBLE_EQ(x(i).val_.val_, get_base1(x, i + 1, "x", 1).val_.val_);
  EXPECT_THROW(get_base1(x, 0, "x", 1), std::out_of_range);
  EXPECT_THROW(get_base1(x, 100, "x", 1), std::out_of_range);
}
