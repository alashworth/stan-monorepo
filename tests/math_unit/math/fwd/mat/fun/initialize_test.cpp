...
TEST(AgradFwdMatrixInitialize, fd) {
  ...
  fvar<double> y = 10;
  initialize(x, y);
  EXPECT_DOUBLE_EQ(10.0, x.val_);

  std::vector<fvar<double> > z(2);
  fvar<double> a = 15;
  initialize(z, a);
  EXPECT_DOUBLE_EQ(15.0, z[0].val_);
  EXPECT_DOUBLE_EQ(15.0, z[1].val_);
  EXPECT_EQ(2U, z.size());
...
TEST(AgradFwdMatrixInitialize, ffd) {
  ...
  initialize(x, y);
  EXPECT_DOUBLE_EQ(10.0, x.val_.val_);

  std::vector<fvar<fvar<double> > > z(2);
  fvar<fvar<double> > a = 15;
  initialize(z, a);
  EXPECT_DOUBLE_EQ(15.0, z[0].val_.val_);
  EXPECT_DOUBLE_EQ(15.0, z[1].val_.val_);
  EXPECT_EQ(2U, z.size());
...
TEST(AgradFwdMatrixInitialize, ffd2) {
  ...
  initialize(y, 3.0);
  EXPECT_EQ(3, y.size());
  EXPECT_DOUBLE_EQ(3.0, y[0].val_.val_);
}
