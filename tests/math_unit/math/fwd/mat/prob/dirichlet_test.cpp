...
TEST(ProbDistributions, fvar_double) {
  ...
  EXPECT_DOUBLE_EQ(0.6931472, stan::math::dirichlet_log(theta, alpha).val_);
  EXPECT_DOUBLE_EQ(0.99344212, stan::math::dirichlet_log(theta, alpha).d_);
  ...
  EXPECT_DOUBLE_EQ(-43.40045, stan::math::dirichlet_log(theta2, alpha2).val_);
  EXPECT_DOUBLE_EQ(2017.2858, stan::math::dirichlet_log(theta2, alpha2).d_);
}
...
TEST(ProbDistributions, fvar_fvar_double) {
  ...
  EXPECT_DOUBLE_EQ(0.6931472,
                   stan::math::dirichlet_log(theta, alpha).val_.val_);
  EXPECT_DOUBLE_EQ(0.99344212, stan::math::dirichlet_log(theta, alpha).d_.val_);
  ...
  EXPECT_DOUBLE_EQ(-43.40045,
                   stan::math::dirichlet_log(theta2, alpha2).val_.val_);
  EXPECT_DOUBLE_EQ(2017.2858,
