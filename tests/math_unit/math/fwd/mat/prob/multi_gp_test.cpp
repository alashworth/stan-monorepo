...
TEST(ProbDistributionsMultiGP, fvar_double) {
  ...
  EXPECT_DOUBLE_EQ(lp_ref.val_, stan::math::multi_gp_log(y, Sigma, w).val_);
  EXPECT_DOUBLE_EQ(-74.572952, stan::math::multi_gp_log(y, Sigma, w).d_);
}
...
TEST(ProbDistributionsMultiGP, fvar_fvar_double) {
  ...
  EXPECT_DOUBLE_EQ(-74.572952, stan::math::multi_gp_log(y, Sigma, w).d_.val_);
}
