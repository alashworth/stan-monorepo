...
TEST(ProbDistributionsCategoricalLogit, fvar_double) {
  ...
  EXPECT_DOUBLE_EQ(theta_log_softmax[0].val_,
                   stan::math::categorical_logit_log(1, theta).val_);
  EXPECT_DOUBLE_EQ(theta_log_softmax[1].val_,
                   stan::math::categorical_logit_log(2, theta).val_);
  EXPECT_DOUBLE_EQ(theta_log_softmax[2].val_,
                   stan::math::categorical_logit_log(3, theta).val_);
  EXPECT_DOUBLE_EQ(theta_log_softmax[0].d_,
                   stan::math::categorical_logit_log(1, theta).d_);
  EXPECT_DOUBLE_EQ(theta_log_softmax[1].d_,
                   stan::math::categorical_logit_log(2, theta).d_);
  EXPECT_DOUBLE_EQ(theta_log_softmax[2].d_,
...
TEST(ProbDistributionsCategoricalLogit, fvar_double_vectorized) {
  ...
  std::vector<int> ns(0);
  EXPECT_DOUBLE_EQ(0.0, stan::math::categorical_logit_log(ns, theta).val_);
  ...
  ms[1] = 2;
  ms[2] = 1;
  EXPECT_DOUBLE_EQ(theta_log_softmax[0].val_ + theta_log_softmax[1].val_
                       + theta_log_softmax[0].val_,
                  stan::math::categorical_logit_log(ms, theta).val_);
  EXPECT_DOUBLE_EQ(theta_log_softmax[0].d_ + theta_log_softmax[1].d_
...
TEST(ProbDistributionsCategoricalLogit, fvar_fvar_double) {
  ...
  Matrix<fvar<fvar<double> >, Dynamic, 1> theta_log_softmax
      = log_softmax(theta);
  EXPECT_DOUBLE_EQ(theta_log_softmax[0].val_.val_,
                   stan::math::categorical_logit_log(1, theta).val_.val_);
  EXPECT_DOUBLE_EQ(theta_log_softmax[1].val_.val_,
                   stan::math::categorical_logit_log(2, theta).val_.val_);
  EXPECT_DOUBLE_EQ(theta_log_softmax[2].val_.val_,
                   stan::math::categorical_logit_log(3, theta).val_.val_);
  EXPECT_DOUBLE_EQ(theta_log_softmax[0].d_.val_,
                   stan::math::categorical_logit_log(1, theta).d_.val_);
  EXPECT_DOUBLE_EQ(theta_log_softmax[1].d_.val_,
                   stan::math::categorical_logit_log(2, theta).d_.val_);
  EXPECT_DOUBLE_EQ(theta_log_softmax[2].d_.val_,
...
TEST(ProbDistributionsCategoricalLogit, fvar_fvar_double_vectorized) {
  ...
  std::vector<int> ns(0);
  EXPECT_DOUBLE_EQ(0.0, stan::math::categorical_logit_log(ns, theta).val_.val_);
  ...
  EXPECT_DOUBLE_EQ(theta_log_softmax[0].d_.val_ + theta_log_softmax[1].d_.val_
