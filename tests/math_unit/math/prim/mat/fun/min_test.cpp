...
TEST(MathMatrix, min) {
  ...
  std::vector<double> x;
  EXPECT_DOUBLE_EQ(std::numeric_limits<double>::infinity(), min(x));
  x.push_back(1.0);
  EXPECT_DOUBLE_EQ(1.0, min(x));
  x.push_back(0.0);
  EXPECT_DOUBLE_EQ(0.0, min(x));
  x.push_back(2.0);
  EXPECT_DOUBLE_EQ(0.0, min(x));
  x.push_back(-10.0);
  EXPECT_DOUBLE_EQ(-10.0, min(x));

  stan::math::vector_d v;
  EXPECT_DOUBLE_EQ(std::numeric_limits<double>::infinity(), min(v));
  v = stan::math::vector_d(1);
  v << 1.0;
  EXPECT_DOUBLE_EQ(1.0, min(v));
  v = stan::math::vector_d(2);
  v << 1.0, 0.0;
  EXPECT_DOUBLE_EQ(0.0, min(v));
  v = stan::math::vector_d(3);
  v << 1.0, 0.0, 2.0;
  EXPECT_DOUBLE_EQ(0.0, min(v));
  v = stan::math::vector_d(4);
  v << 1.0, 0.0, 2.0, -10.0;
  EXPECT_DOUBLE_EQ(-10.0, min(v));

  stan::math::row_vector_d rv;
  EXPECT_DOUBLE_EQ(std::numeric_limits<double>::infinity(), min(rv));
  rv = stan::math::row_vector_d(1);
  rv << 1.0;
  EXPECT_DOUBLE_EQ(1.0, min(rv));
  rv = stan::math::row_vector_d(2);
  rv << 1.0, 0.0;
  EXPECT_DOUBLE_EQ(0.0, min(rv));
  rv = stan::math::row_vector_d(3);
  rv << 1.0, 0.0, 2.0;
  EXPECT_DOUBLE_EQ(0.0, min(rv));
  rv = stan::math::row_vector_d(4);
  rv << 1.0, 0.0, 2.0, -10.0;
  EXPECT_DOUBLE_EQ(-10.0, min(rv));

  stan::math::matrix_d m;
  EXPECT_DOUBLE_EQ(std::numeric_limits<double>::infinity(), min(m));
  m = stan::math::matrix_d(1, 1);
  m << 1.0;
  EXPECT_DOUBLE_EQ(1.0, min(m));
  m = stan::math::matrix_d(2, 2);
  m << 1.0, 0.0, 2.0, -10.0;
  EXPECT_DOUBLE_EQ(-10.0, min(m));
}
