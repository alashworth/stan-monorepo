...
TEST(MathPrimMat, nan_error_training_sig_l_1) {
  double sigma = 0.2;
  double l = 5;
...
TEST(MathPrimMat, check_dim_mismatch_0) {
  double sig = 1.0;
  double l = 1.0;
...
TEST(MathPrimMat, calculations_0) {
  double sigma = 1.0;
  double l = 1.0;
...
TEST(MathPrimMat, calculations_ard_0) {
  double sigma = 1.0;
  ...
  EXPECT_DOUBLE_EQ(exp(-sqrt(1 + 9.0 / 4.0)), cov(1, 0));
  EXPECT_DOUBLE_EQ(exp(-sqrt(1 + 9.0 / 4.0)), cov2(1, 0));
  EXPECT_DOUBLE_EQ(exp(-sqrt(1 + 9.0 / 4.0)), cov(0, 1));
  EXPECT_DOUBLE_EQ(exp(-sqrt(1 + 9.0 / 4.0)), cov2(0, 1));
