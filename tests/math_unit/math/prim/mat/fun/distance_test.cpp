...
TEST(MathMatrix, distance_vector_vector) {
  ...
  EXPECT_DOUBLE_EQ(7.071068, stan::math::distance(v1, v2));

  v1.resize(0);
  v2.resize(0);
  EXPECT_DOUBLE_EQ(0, stan::math::distance(v1, v2));
...
TEST(MathMatrix, distance_rowvector_vector) {
  ...
  rv << 1, 3, -5;
  v << 4, -2, -1;
  EXPECT_DOUBLE_EQ(7.071068, stan::math::distance(rv, v));

  rv.resize(0);
  v.resize(0);
  EXPECT_DOUBLE_EQ(0, stan::math::distance(rv, v));
...
TEST(MathMatrix, distance_vector_rowvector) {
  ...
  v << 1, 3, -5;
  rv << 4, -2, -1;
  EXPECT_DOUBLE_EQ(7.071068, stan::math::distance(v, rv));

  v.resize(0);
  rv.resize(0);
  EXPECT_DOUBLE_EQ(0, stan::math::distance(v, rv));
