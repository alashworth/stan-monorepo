...
void test_cumulative_sum() {
  ...
  EXPECT_DOUBLE_EQ(e[0], f[0]);
  EXPECT_DOUBLE_EQ(e[0] + e[1], f[1]);
  ...
  EXPECT_DOUBLE_EQ(g[0], h[0]);
  EXPECT_DOUBLE_EQ(g[0] + g[1], h[1]);
  EXPECT_DOUBLE_EQ(g[0] + g[1] + g[2], h[2]);
}

TEST(MathMatrix, cumulative_sum) {
  using stan::math::cumulative_sum;

  EXPECT_DOUBLE_EQ(0, cumulative_sum(std::vector<double>(0)).size());
