...
TEST(MetaTraits, StdVectorBuilder_false_false_1) {
  using Eigen::Dynamic;
  using Eigen::Matrix;
...
TEST(MetaTraits, StdVectorBuilder_true_false_1) {
  using Eigen::Dynamic;
  using Eigen::Matrix;
  using stan::StdVectorBuilder;
  using stan::length;
  ...
  EXPECT_DOUBLE_EQ(0.0, dvv3[0]);
  EXPECT_DOUBLE_EQ(0.0, dvv3[1]);
  EXPECT_DOUBLE_EQ(0.0, dvv3[2]);
  double data3 = 0.0;
  EXPECT_NO_THROW(data3 = dvv3.data());
  EXPECT_DOUBLE_EQ(0.0, data3);

  StdVectorBuilder<true, double, double> dvv4(length(a_row_vector));
  EXPECT_DOUBLE_EQ(0.0, dvv4[0]);
  EXPECT_DOUBLE_EQ(0.0, dvv4[1]);
  EXPECT_DOUBLE_EQ(0.0, dvv4[2]);
  double data4 = 0.0;
  EXPECT_NO_THROW(data4 = dvv4.data());
  EXPECT_DOUBLE_EQ(0.0, data4);
}

TEST(MetaTraits, StdVectorBuilder_type_check_1) {
  using stan::StdVectorBuilder;
