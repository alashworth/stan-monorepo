...
TEST(ProbDirichlet, log_matches_lpmf) {
  ...
  EXPECT_DOUBLE_EQ((stan::math::dirichlet_lpmf(theta, alpha)),
                   (stan::math::dirichlet_log(theta, alpha)));
  EXPECT_DOUBLE_EQ((stan::math::dirichlet_lpmf<true>(theta, alpha)),
                   (stan::math::dirichlet_log<true>(theta, alpha)));
  EXPECT_DOUBLE_EQ((stan::math::dirichlet_lpmf<false>(theta, alpha)),
