...
TEST(prob_transform, offset_multiplier) {
  EXPECT_DOUBLE_EQ(2.0 - 5.0 * 1.0,
                   stan::math::offset_multiplier_constrain(-1.0, 2.0, 5.0));

  EXPECT_DOUBLE_EQ(1.7, stan::math::offset_multiplier_constrain(1.7, 0, 1));
}
TEST(prob_transform, offset_multiplier_j) {
  ...
  double U = 5.0;
  double x = -1.0;
  EXPECT_DOUBLE_EQ(L + U * x,
                   stan::math::offset_multiplier_constrain(x, L, U, lp));
  EXPECT_DOUBLE_EQ(-17.0 + log(U), lp);

  double lp1 = -12.9;
  EXPECT_DOUBLE_EQ(1.7,
                   stan::math::offset_multiplier_constrain(1.7, 0, 1, lp1));
...
TEST(prob_transform, offset_multiplier_f) {
  ...
  double U = 27.0;
  double y = 3.0;
  EXPECT_DOUBLE_EQ(y, stan::math::offset_multiplier_constrain(
