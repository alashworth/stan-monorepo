...
TEST(MathFunctions, log_inv_logit) {
  ...
  EXPECT_DOUBLE_EQ(log(inv_logit(-7.2)), log_inv_logit(-7.2));
  EXPECT_DOUBLE_EQ(log(inv_logit(0.0)), log_inv_logit(0.0));
  EXPECT_DOUBLE_EQ(log(inv_logit(1.9)), log_inv_logit(1.9));
}
