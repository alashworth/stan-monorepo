...
TEST(prob_transform, ub) {
  EXPECT_DOUBLE_EQ(2.0 - exp(-1.0), stan::math::ub_constrain(-1.0, 2.0));
  EXPECT_DOUBLE_EQ(1.7, stan::math::ub_constrain(
                            1.7, std::numeric_limits<double>::infinity()));
}
TEST(prob_transform, ub_j) {
  double lp = 15.0;
  EXPECT_DOUBLE_EQ(2.0 - exp(-1.0), stan::math::ub_constrain(-1.0, 2.0, lp));
  ...
  EXPECT_DOUBLE_EQ(1.87, lp2);
}
TEST(prob_transform, ub_f) {
  double y = 2.0;
  double U = 4.0;
  EXPECT_DOUBLE_EQ(log(-(y - U)), stan::math::ub_free(2.0, 4.0));
