...
void test_binom_coefficient(const T_N& N, const T_n& n) {
  ...
  EXPECT_DOUBLE_EQ(lgamma(N + 1) - lgamma(n + 1) - lgamma(N - n + 1),
                   binomial_coefficient_log(N, n));
}

TEST(MathFunctions, binomial_coefficient_log) {
  using stan::math::binomial_coefficient_log;
  EXPECT_DOUBLE_EQ(1.0, exp(binomial_coefficient_log(2.0, 2.0)));
  EXPECT_DOUBLE_EQ(2.0, exp(binomial_coefficient_log(2.0, 1.0)));
  EXPECT_DOUBLE_EQ(3.0, exp(binomial_coefficient_log(3.0, 1.0)));
  EXPECT_NEAR(3.0, exp(binomial_coefficient_log(3.0, 2.0)), 0.0001);

  EXPECT_DOUBLE_EQ(29979.16, binomial_coefficient_log(100000, 91116));
