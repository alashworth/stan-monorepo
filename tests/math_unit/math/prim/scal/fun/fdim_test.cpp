...
TEST(MathFunctions, fdim_double) {
  ...
  EXPECT_DOUBLE_EQ(1.0, fdim(3.0, 2.0));
  EXPECT_DOUBLE_EQ(0.0, fdim(2.0, 3.0));

  EXPECT_DOUBLE_EQ(2.5, fdim(4.5, 2.0));
}
...
TEST(MathFunctions, fdim_int) {
  ...
  EXPECT_DOUBLE_EQ(1.0, fdim(static_cast<int>(3), static_cast<int>(2)));
  EXPECT_DOUBLE_EQ(0.0, fdim(static_cast<int>(2), static_cast<int>(3)));
}
