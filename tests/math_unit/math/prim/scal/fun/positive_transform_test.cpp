...
TEST(prob_transform, positive) {
  EXPECT_DOUBLE_EQ(exp(-1.0), stan::math::positive_constrain(-1.0));
}
TEST(prob_transform, positive_j) {
  double lp = 15.0;
  EXPECT_DOUBLE_EQ(exp(-1.0), stan::math::positive_constrain(-1.0, lp));
  EXPECT_DOUBLE_EQ(15.0 - 1.0, lp);
}
TEST(prob_transform, positive_f) {
  EXPECT_DOUBLE_EQ(log(0.5), stan::math::positive_free(0.5));
}
