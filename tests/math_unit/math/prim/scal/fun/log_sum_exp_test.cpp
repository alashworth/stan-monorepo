...
void test_log_sum_exp(double a, double b) {
  ...
  using std::exp;
  using std::log;
  EXPECT_DOUBLE_EQ(log(exp(a) + exp(b)), log_sum_exp(a, b));
}
...
TEST(MathFunctions, log_sum_exp_1) {
  ...
  as.push_back(10000.0);
  EXPECT_DOUBLE_EQ(10000.0, log_sum_exp(as));
}
...
TEST(MathFunctions, log_sum_exp_2) {
  ...
  EXPECT_DOUBLE_EQ(10000.0, log_sum_exp(10000.0, 0.0));
  EXPECT_DOUBLE_EQ(0.0, log_sum_exp(-10000.0, 0.0));
}
