...
TEST(MathFunctions, expm1) {
  ...
  using std::exp;
  EXPECT_DOUBLE_EQ(exp(-14.2) - 1, expm1(-14.2));
  EXPECT_DOUBLE_EQ(exp(0) - 1, expm1(0));
  EXPECT_DOUBLE_EQ(exp(172.987) - 1, expm1(172.987));
  EXPECT_DOUBLE_EQ(-1, expm1(-std::numeric_limits<double>::infinity()));
}
