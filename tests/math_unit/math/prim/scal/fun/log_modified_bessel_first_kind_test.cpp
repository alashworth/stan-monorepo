...
TEST(MathFunctions, log_modified_bessel_first_kind) {
  ...
  EXPECT_DOUBLE_EQ(log_modified_bessel_first_kind(sqrt(3), sqrt(2)),
                   log(boost::math::cyl_bessel_i(sqrt(3), sqrt(2))));

  EXPECT_DOUBLE_EQ(log_modified_bessel_first_kind(0.5, 10),
  ...
  EXPECT_DOUBLE_EQ(log_modified_bessel_first_kind(.1, 100),
                   log_modified_bessel_first_kind(.1, 100 + 1e-14));

  // code branches when v is -1
  EXPECT_DOUBLE_EQ(-0.5706479874908312, log_modified_bessel_first_kind(-1, 1));
  EXPECT_DOUBLE_EQ(-0.5706479874908312,
                   log_modified_bessel_first_kind(-1 + 1e-16, 1));

  // code branches at v = 0
  EXPECT_DOUBLE_EQ(0.235914358507179,
                   log_modified_bessel_first_kind(0, 1));  // integers promoted
  EXPECT_DOUBLE_EQ(0.235914358507179, log_modified_bessel_first_kind(1e-16, 1));

  EXPECT_DOUBLE_EQ(5.82456472298118,
                   log_modified_bessel_first_kind(0, 7.75 - 5e-16));
  EXPECT_DOUBLE_EQ(5.82456472298118, log_modified_bessel_first_kind(0, 7.75));
  EXPECT_DOUBLE_EQ(5.82456472298118,
                   log_modified_bessel_first_kind(1e-16, 7.75));

  EXPECT_DOUBLE_EQ(495.974007668107,
                   log_modified_bessel_first_kind(0, 500 - 1e-13));
  EXPECT_DOUBLE_EQ(495.974007668107, log_modified_bessel_first_kind(0, 500));
  EXPECT_DOUBLE_EQ(495.974007668107,
                   log_modified_bessel_first_kind(1e-16, 500));

  // code branches at v = 1
  EXPECT_DOUBLE_EQ(-0.570647987490831,
                   log_modified_bessel_first_kind(1, 1));  // integers promoted
  EXPECT_DOUBLE_EQ(-0.570647987490831,
                   log_modified_bessel_first_kind(1 + 1e-16, 1));

  EXPECT_DOUBLE_EQ(5.75527527206771,
                   log_modified_bessel_first_kind(1, 7.75 - 5e-16));
  EXPECT_DOUBLE_EQ(5.75527527206771, log_modified_bessel_first_kind(1, 7.75));
  EXPECT_DOUBLE_EQ(5.75527527206771,
                   log_modified_bessel_first_kind(1 + 1e-16, 7.75));

  EXPECT_DOUBLE_EQ(495.973006666268,
                   log_modified_bessel_first_kind(1, 500 - 1e-13));
  EXPECT_DOUBLE_EQ(495.973006666268, log_modified_bessel_first_kind(1, 500));
  EXPECT_DOUBLE_EQ(495.973006666268,
