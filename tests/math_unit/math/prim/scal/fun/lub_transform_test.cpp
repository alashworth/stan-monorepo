...
TEST(prob_transform, lub) {
  EXPECT_DOUBLE_EQ(2.0 + (5.0 - 2.0) * stan::math::inv_logit(-1.0),
                   stan::math::lub_constrain(-1.0, 2.0, 5.0));

  EXPECT_DOUBLE_EQ(1.7, stan::math::lub_constrain(
                            1.7, -std::numeric_limits<double>::infinity(),
                           +std::numeric_limits<double>::infinity()));
  EXPECT_DOUBLE_EQ(stan::math::lb_constrain(1.8, 3.0),
                   stan::math::lub_constrain(
                      1.8, 3.0, +std::numeric_limits<double>::infinity()));
  EXPECT_DOUBLE_EQ(stan::math::ub_constrain(1.9, -12.5),
                   stan::math::lub_constrain(
...
TEST(prob_transform, lub_j) {
  ...
  double U = 5.0;
  double x = -1.0;
  EXPECT_DOUBLE_EQ(L + (U - L) * stan::math::inv_logit(x),
                   stan::math::lub_constrain(x, L, U, lp));
  EXPECT_DOUBLE_EQ(-17.0 + log(U - L) + log(stan::math::inv_logit(x))
  ...
  EXPECT_DOUBLE_EQ(lp2_expected, lp2);
  ...
  EXPECT_DOUBLE_EQ(
      ...
      stan::math::lub_constrain(1.9, -std::numeric_limits<double>::infinity(),
                                -12.5, lp3));
  EXPECT_DOUBLE_EQ(lp3_expected, lp3);
}
