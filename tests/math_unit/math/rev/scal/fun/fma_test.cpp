...
TEST(AgradRev, fma_vvv) {
  ...
  AVAR c = 7.0;
  AVAR f = fma(a, b, c);
  EXPECT_DOUBLE_EQ(3.0 * 5.0 + 7.0, f.val());

  AVEC x = createAVEC(a, b, c);
  VEC grad_f;
  f.grad(x, grad_f);
  EXPECT_DOUBLE_EQ(5.0, grad_f[0]);
  EXPECT_DOUBLE_EQ(3.0, grad_f[1]);
  EXPECT_DOUBLE_EQ(1.0, grad_f[2]);
}
...
TEST(AgradRev, check_varis_on_stack_203) {
  AVAR a = 3.0;
  AVAR b = 5.0;
  AVAR c = 7.0;
