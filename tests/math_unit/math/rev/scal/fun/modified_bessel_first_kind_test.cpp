...
TEST(AgradRev, modified_bessel_first_kind_int_var) {
  ...
  EXPECT_DOUBLE_EQ(9.75946515370444990947519256731268090, f.val());

  AVEC x = createAVEC(a, b);
  VEC g;
  f.grad(x, g);
  EXPECT_DOUBLE_EQ(0, g[0]);
  EXPECT_DOUBLE_EQ(8.862055663710218018987472041388932, g[1]);

  a = -1;
  b = -3.0;
  f = stan::math::modified_bessel_first_kind(a, b);

  EXPECT_DOUBLE_EQ(-3.95337021740260939647863574058058, f.val());

  x = createAVEC(a, b);
  f.grad(x, g);
  EXPECT_DOUBLE_EQ(0, g[0]);
  EXPECT_DOUBLE_EQ(3.5630025133974876201183569658, g[1]);
}
