...
TEST(AgradRev, fdim_vv) {
  ...
  EXPECT_DOUBLE_EQ(0.0, fdim(a, a).val());
  EXPECT_DOUBLE_EQ(std::numeric_limits<double>::infinity(), fdim(a, b).val());
  EXPECT_DOUBLE_EQ(0.0, fdim(b, a).val());
}
...
TEST(AgradRev, fdim_vd) {
  ...
  EXPECT_DOUBLE_EQ(0.0, fdim(infinityavar, infinitydouble).val());
  EXPECT_DOUBLE_EQ(std::numeric_limits<double>::infinity(),
                   fdim(infinityavar, b).val());
  EXPECT_DOUBLE_EQ(0.0, fdim(a, infinitydouble).val());
}
...
TEST(AgradRev, fdim_dv) {
  ...
  AVAR infinityavar = std::numeric_limits<AVAR>::infinity();
  double infinitydouble = std::numeric_limits<double>::infinity();
  EXPECT_DOUBLE_EQ(0.0, fdim(infinitydouble, infinityavar).val());
  EXPECT_DOUBLE_EQ(std::numeric_limits<double>::infinity(),
                   fdim(infinitydouble, b).val());
  EXPECT_DOUBLE_EQ(0.0, fdim(a, infinityavar).val());
}
