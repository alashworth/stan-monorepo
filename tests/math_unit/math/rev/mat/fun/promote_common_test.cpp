...
TEST(AgradRevMatrix, promote_common_scal) {
  ...
  EXPECT_DOUBLE_EQ(4.0, d.val());
  EXPECT_DOUBLE_EQ(4.0, e.val());
}
...
TEST(AgradRevMatrix, promote_common_matrix_d) {
  ...
  for (int i = 0; i < 3; ++i)
    for (int j = 0; j < 4; ++j)
      EXPECT_DOUBLE_EQ(A(i, j), E(i, j).val());
}
