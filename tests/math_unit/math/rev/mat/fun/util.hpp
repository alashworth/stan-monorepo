...
static inline double relative_diff(double u, double v) {
  return 2 * (u - v) / (fabs(u) + fabs(v));
}

static inline void expect_near_relative(double u, double v) {
  if (u == v)
    SUCCEED();
  else if (u == 0 || v == 0)
...
static inline Eigen::Matrix<double, 3, 3> third_order_mixed_hess(
...
static inline Eigen::Matrix<double, 3, 3> norm_hess(
    const Eigen::Matrix<double, Eigen::Dynamic, 1>& inp_vec) {
  using Eigen::Dynamic;
  using Eigen::Matrix;
