...
TEST(AgradRevMatrix, divide_scalar) {
  ...
  EXPECT_DOUBLE_EQ(-5, divide(d1, v2).val());
  EXPECT_DOUBLE_EQ(-5, divide(v1, d2).val());
  EXPECT_DOUBLE_EQ(-5, divide(v1, v2).val());
...
TEST(AgradRevMatrix, divide_vector) {
  ...
  vector_v output;
  output = divide(d1, v2);
  EXPECT_DOUBLE_EQ(-50, output(0).val());
  EXPECT_DOUBLE_EQ(0, output(1).val());
  EXPECT_DOUBLE_EQ(1.5, output(2).val());
  ...
  output = divide(d1, v2);
  EXPECT_DOUBLE_EQ(std::numeric_limits<double>::infinity(), output(0).val());
  EXPECT_TRUE(std::isnan(output(1).val()));
  EXPECT_DOUBLE_EQ(-std::numeric_limits<double>::infinity(), output(2).val());
...
TEST(AgradRevMatrix, divide_matrix) {
  ...
  matrix_v output;
  output = divide(d1, v2);
  EXPECT_DOUBLE_EQ(-50, output(0, 0).val());
  EXPECT_DOUBLE_EQ(0, output(0, 1).val());
  EXPECT_DOUBLE_EQ(1.5, output(1, 0).val());
  EXPECT_DOUBLE_EQ(-2, output(1, 1).val());
  ...
  output = divide(d1, v2);
  EXPECT_DOUBLE_EQ(std::numeric_limits<double>::infinity(), output(0, 0).val());
  EXPECT_TRUE(std::isnan(output(0, 1).val()));
  EXPECT_DOUBLE_EQ(-std::numeric_limits<double>::infinity(),
                   output(1, 0).val());
  EXPECT_DOUBLE_EQ(std::numeric_limits<double>::infinity(), output(1, 1).val());
...
TEST(AgradRevMatrix, check_varis_on_stack_7) {
