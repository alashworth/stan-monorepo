...
TEST(AgradRevMatrix, elt_multiply_vec_vv) {
  ...
  EXPECT_DOUBLE_EQ(20.0, z(0).val());
  EXPECT_DOUBLE_EQ(500.0, z(1).val());

  VEC g = cgradvec(z(0), x_ind);
  EXPECT_DOUBLE_EQ(10.0, g[0]);
  EXPECT_DOUBLE_EQ(0.0, g[1]);
  EXPECT_DOUBLE_EQ(2.0, g[2]);
...
TEST(AgradRevMatrix, elt_multiply_matrix_vv) {
  ...
  EXPECT_DOUBLE_EQ(20.0, z(0, 0).val());
  EXPECT_DOUBLE_EQ(500.0, z(0, 1).val());
  EXPECT_DOUBLE_EQ(29000000.0, z(1, 2).val());
...
TEST(AgradRevMatrix, check_varis_on_stack_12) {
  using stan::math::elt_multiply;
