...
TEST(MetaTraits, VectorBuilderHelper_false_true_1) {
  using Eigen::Dynamic;
  using Eigen::Matrix;
...
TEST(MetaTraits, VectorBuilderHelper_true_true_1) {
  using Eigen::Dynamic;
  using Eigen::Matrix;
  using stan::VectorBuilderHelper;
  using stan::length;
  ...
  EXPECT_DOUBLE_EQ(1.0, dvv3[1]);
  EXPECT_DOUBLE_EQ(2.0, dvv3[2]);
  ...
  EXPECT_DOUBLE_EQ(1.0, dvv4[1]);
  EXPECT_DOUBLE_EQ(2.0, dvv4[2]);
