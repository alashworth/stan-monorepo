...
TEST_F(StanAgradRevOde, coupled_ode_system_dv) {
  ...
  EXPECT_DOUBLE_EQ(0.5, dz_dt[0]);
  EXPECT_DOUBLE_EQ(-1.075, dz_dt[1]);
  EXPECT_DOUBLE_EQ(2, dz_dt[2]);
  EXPECT_DOUBLE_EQ(-1.8, dz_dt[3]);
...
TEST_F(StanAgradRevOde, coupled_ode_system_vd) {
  ...
  EXPECT_DOUBLE_EQ(0.5, dz_dt[0]);
  EXPECT_DOUBLE_EQ(-1.0 - 0.15 * 0.5, dz_dt[1]);
  EXPECT_DOUBLE_EQ(0.0 * 1.0 + 1.0 * 0.0, dz_dt[2]);
  EXPECT_DOUBLE_EQ(-1.0 * 1.0 - 0.15 * 0.0, dz_dt[3]);
  EXPECT_DOUBLE_EQ(0.0 * 0.0 + 1.0 * 1.0, dz_dt[4]);
  EXPECT_DOUBLE_EQ(-1.0 * 0.0 - 0.15 * 1.0, dz_dt[5]);
...
TEST_F(StanAgradRevOde, initial_state_vd) {
  ...
  for (size_t n = 0; n < N; n++)
    EXPECT_DOUBLE_EQ(n + 1, state[n]);
  for (size_t i = 0; i < N; i++)
    for (size_t j = 0; j < N; j++)
      EXPECT_DOUBLE_EQ(i == j ? 1.0 : 0.0, state[N + i + j * N]);
}
...
TEST_F(StanAgradRevOde, coupled_ode_system_vv) {
  ...
  EXPECT_DOUBLE_EQ(dy_dt_base[0], dz_dt[0]);
  EXPECT_DOUBLE_EQ(dy_dt_base[1], dz_dt[1]);
  EXPECT_DOUBLE_EQ(0, dz_dt[2]);
  EXPECT_DOUBLE_EQ(-1, dz_dt[3]);
  EXPECT_DOUBLE_EQ(1, dz_dt[4]);
  EXPECT_DOUBLE_EQ(-0.15, dz_dt[5]);
  EXPECT_DOUBLE_EQ(0, dz_dt[6]);
  EXPECT_DOUBLE_EQ(-0.5, dz_dt[7]);
