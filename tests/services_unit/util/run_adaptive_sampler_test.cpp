...
#include <diagnose/test_lp.hpp>
#include <stan/util/io/empty_var_context.hpp>
#include <stan/services/util/create_rng.hpp>
#include "instrumented_callbacks.hpp"
#include <stan/algorithms/hmc/nuts/adapt_unit_e_nuts.hpp>

class AdaptiveSampler : public testing::Test {
public:
  AdaptiveSampler()
...
TEST_F(AdaptiveSampler, all_zero) {
...
TEST_F(AdaptiveSampler, num_warmup_no_save) {
  num_warmup = 1000;
  stan::services::util::run_adaptive_sampler(sampler, model,
                                             cont_vector,
                                             num_warmup, num_samples,
...
TEST_F(AdaptiveSampler, num_warmup_save) {
  num_warmup = 1000;
  save_warmup = true;
  stan::services::util::run_adaptive_sampler(sampler, model,
                                             cont_vector,
...
TEST_F(AdaptiveSampler, num_samples) {
  num_samples = 1000;
  stan::services::util::run_adaptive_sampler(sampler, model,
                                             cont_vector,
                                             num_warmup, num_samples,
...
TEST_F(AdaptiveSampler, num_warmup_save_num_samples_num_thin) {
  num_warmup = 500;
  save_warmup = true;
  num_samples = 500;
  num_thin = 10;
...
TEST_F(AdaptiveSampler, num_warmup_num_samples_refresh) {
  num_warmup = 500;
  num_samples = 500;
  refresh = 10;
  stan::services::util::run_adaptive_sampler(sampler, model,
