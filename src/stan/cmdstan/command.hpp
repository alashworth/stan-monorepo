...
#include "arguments/arg_data.hpp"
#include "arguments/arg_id.hpp"
#include "arguments/arg_init.hpp"
#include "arguments/arg_output.hpp"
#include "arguments/arg_random.hpp"
#include "arguments/argument_parser.hpp"
#include "write_model.hpp"
#include "write_stan.hpp"
#include <stan/language/model/model_base.hpp>
...
std::shared_ptr<stan::io::var_context> get_var_context(const std::string file) {
  ...
  if (stan::io::ends_with(".json", file)) {
    ...
    std::shared_ptr<stan::io::var_context> result
        = std::make_shared<stan::json::json_data>(var_context);
    return result;
  }
...
int command(int argc, const char* argv[]) {
  ...
  int_argument* random_arg
      = dynamic_cast<int_argument*>(parser.arg("random")->arg("seed"));
  if (random_arg->is_default()) {
    random_arg->set_value(
  ...
  int random_seed
      = dynamic_cast<int_argument*>(parser.arg("random")->arg("seed"))->value();
